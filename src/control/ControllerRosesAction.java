 ...
 */
public class ControllerRosesAction extends ControllerAction implements EventHandler<ActionEvent> {

    // to avoid lots of casts, create an attribute that matches the instance type.
    private RosesView holeView;

    public ControllerRosesAction(Model model, View view, Controller control) {
        super(model, view, control);
        // take the view parameter ot define a local view attribute with the real instance type, i.e. BasicView.
        holeView = (RosesView) view;
    ...
    private void setMenuHandlers() {
        ...
        holeView.getMenuStart().setOnAction(e -> {
            try {
                control.startGame();
                holeView.getStage().setFullScreen(true);
                holeView.getStage().setResizable(false);
